		...
		<view class="recom_area">
			<view class="simgle-item" v-for="itemObject in recommd_items">					
				...
				<view class="rcm-item-desc">
					<!-- 教程页面webview跳转 
					参考：https://www.jianshu.com/p/adc72eae0593
					-->					
					<view class="rcm-title" @click="goto_webview(itemObject)">
						{{itemObject.title}}
					</view>									
					<view class="rcm-info">
						<text style="color: gray;margin-right: 5px;">来自</text>
						<u-link :href="itemObject.res_from_url" >{{itemObject.res_from}}</u-link>						
					</view>	
					<view class="rcm-info">
...
<script>
	const recmd_items_data = require('@/common/json/recmd_items_course.json');
	export default {
		data() {
			return {
				my_src:'https://ss1.bdstatic.com/70cFvXSh_Q1YnxGkpoWK1HF6hhy/it/u=3415312089,2997516152&fm=26&gp=0.jpg',
				reason_kws : [],
				recommd_items:[],					
			}
		},
		methods: {
			...
			goto_webview(data){
				let u_data = {key:'url',value:data.course_url};
				uni.setStorage({
							key:'setUrl',
							data:u_data
							});
				const u = `../web_view_show/web_view_show?url=${encodeURIComponent(data.course_url)}`
				const u2 = '../web_view_show/web_view_show?url='+data.course_url;
				debugger;
				uni.navigateTo({
					url:'../web_view_show/web_view_show?url='+data.course_url																		
				});							
			}
	},
	}
